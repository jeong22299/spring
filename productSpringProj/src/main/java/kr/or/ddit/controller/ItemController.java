...
public class ItemController {
	...
	private String uploadPath = "C:\\eclipse-jee-2020-06-R-win32-x86_64\\workspace\\productSpringProj\\src\\main\\webapp\\resources\\upload";
	...
	// 다증 이미지 업로드
	// 요청URI : /uploadFormAction
	// <input type="file" name="uploadFile" multiple>
	//***중요*** name="uploadFile"과 MultipartFile[] uploadFile은 동일해야함
	...
	public String uploadFormAction(MultipartFile[] uploadFile, Model model) {
		...
		/*
			...
			-- 잘 안쓰는 방법
			String.. = file.getName()
			byte[].. = file.getByte() : byte[]로 파일 데이터 변환
			InputStream.. = getInputStram : 파일 데이터와 연결된 InputStream을 반환
		 */
	...
	public boolean checkImageType(File file) {
		...
		try {
			...
			//MINE 타입 객체가 image로 시작하는지 여부를 return
